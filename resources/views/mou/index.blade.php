                ...
                <table class="table table-bordered" id="MouTable">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">{{ $mou->tahunAkademik->tahun ?? '-' }} {{ $mou->tahunAkademik->ganjil_genap ?? '-' }}</td>
                            ...
                            <td class="text-center">
                                <div class="btn-group" role="group">
                                    <a href="{{ route('mou.show', $mou->id_mou) }}" class="btn btn-outline-primary btn-sm">
                                    ...
                                    <button class="btn btn-outline-danger btn-sm" onclick="confirmDelete('{{ $mou->id_mou }}', '{{ $mou->no_mou }}')">
...
<script>
    ...
    function editMou(id_mou, no_mou, pihak_1, pihak_2, tanggal_mulai, tanggal_berakhir, id_tahun_akademik, jenis_kerjasama, kontak, file_mou) {
        ...
        // console.log("ID Tahun Akademik:", id_tahun_akademik);

        // Validasi dropdown Tahun Akademik
        if (id_tahun_akademik) {
            if ($("#edit_tahun option[value='" + id_tahun_akademik + "']").length > 0) {
                $('#edit_tahun').val(id_tahun_akademik);
            } else {
                $('#edit_tahun').val('');
            }
        } else {
        ...
        // console.log("Dropdown selected value:", $('#edit_tahun').val());

        // Menampilkan link file MoU jika ada
        let fileMouLink = $('#edit_file_mou_link');
        if (file_mou) {
            fileMouLink.attr("href", file_mou).text("Lihat Dokumen");
        } else {
            fileMouLink.text("Tidak ada dokumen").attr("href", "#");
        }

        // Set action pada form edit MoU
        $('#editMoUForm').attr("action", `/mou/${id_mou}`);

        // Menampilkan modal edit
        $('#editMoUModal').modal('show');
        // Reset input file agar bisa diunggah ulang
        $('#edit_file_mou').val('');

        let dokLink = $('#edit_file_mou_link');
        if (file_mou) {
