            ...
            <div class="card-body">
                ...
                <div class="mt-4 justify-content-between d-flex align-items-center">
                    <form action="{{ route('mahasiswa.filter') }}" method="GET" id="tahunForm">
                        <div>
                            <select class="pr-5 form-control" id="tahunInput" name="tahun_masuk" required onchange="this.form.submit()">
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <button type="button" class="h-10 mr-2 btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal">
                        Tambah Mahasiswa
                        </button>
                                    ...
                                    <form action="{{ route('mahasiswa.store') }}" method="post">
                                        ...
                                        <div class="modal-body">
                                            <div class="mb-3">
                                                ...
                                                <input type="text" class="form-control" id="namaInput" name="nama_mahasiswa" required>
                                            </div>
                                            <div class="mb-3">
                                                <label for="nimInput" class="form-label">NIM Mahasiswa</label>
                                                <input type="text" class="form-control" id="nimInput" name="nim" required>
                                            </div>
                                            <div class="mb-3">
                                                <label for="noInput" class="form-label">Nomor Hp</label>
                                                <input type="text" class="form-control" id="noInput" name="no_hp" required>
                                            </div>
                                            <div class="mb-3">
                                                <label for="noOrtuInput" class="form-label">Nomor Hp Orang Tua</label>
                                                <input type="text" class="form-control" id="noOrtuInput" name="no_ortu" required>
                                            </div>
                                            <div class="mb-3">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select class="form-control" id="tahunInput" name="tahun_masuk" required>
                                                    ...
                                                    @foreach ($tahunAkademiks as $x)
                                                        <option value="{{ $x->id_tahun_akademik }}">{{ $x->tahun }} {{ $x->ganjil_genap }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                        ...
                        <form action={{ route('mahasiswa.cari') }} method="GET" class="flex items-center p-2 space-x-2 rounded-lg">
                            <input
                                type="text"
                                name="cari"
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group" role="group">
                                            ...
                                            <div class="modal fade" id="modal-{{ $mahasiswa->id_mahasiswa }}" tabindex="-1" aria-labelledby="modalLabel-{{ $mahasiswa->id_mahasiswa }}" aria-hidden="true">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title" id="modalLabel-{{ $mahasiswa->id_mahasiswa }}">Edit Data Mahasiswa</h5>
                                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                        </div>
                                                        <form action="{{ route('mahasiswa.edit', $mahasiswa->id_mahasiswa) }}" method="POST">
                                                             @csrf
                                                            <div class="modal-body text-start">
                                                                <div class="mb-3">
                                                                    <label for="namaInput-{{ $mahasiswa->id_mahasiswa }}" class="form-label">Nama Mahasiswa</label>
                                                                    <input type="hidden" name="id_mahasiswa" value="{{ $mahasiswa->id_mahasiswa }}">
                                                                    <input type="text" class="form-control" id="namaInput-{{ $mahasiswa->id_mahasiswa }}" name="nama_mahasiswa" value="{{ $mahasiswa->nama_mahasiswa }}">
                                                                </div>
                                                                <div class="mb-3">
                                                                    <label for="noInput-{{ $mahasiswa->id_mahasiswa }}" class="form-label">Nomor Hp</label>
                                                                    <input type="text" class="form-control" id="noInput-{{ $mahasiswa->id_mahasiswa }}" name="no_hp" value="{{ $mahasiswa->no_hp }}">
                                                                </div>
                                                                <div class="mb-3">
                                                                    <label for="noOrtuInput-{{ $mahasiswa->id_mahasiswa }}" class="form-label">Nomor Hp Orang Tua</label>
                                                                    <input type="text" class="form-control" id="noOrtuInput-{{ $mahasiswa->id_mahasiswa }}" name="no_ortu" value="{{ $mahasiswa->no_ortu }}">
                                                                </div>
                                                                <div class="mb-3">
                                                                    <label for="alamatInput-{{ $mahasiswa->id_mahasiswa }}" class="form-label">Alamat</label>
                                                                    <input type="text" class="form-control" id="alamatInput-{{ $mahasiswa->id_mahasiswa }}" name="alamat" value="{{ $mahasiswa->alamat }}">
                                                                </div>
                                                                <div class="mb-3">
                                                                    <label for="statusInput-{{ $mahasiswa->id_mahasiswa }}" class="form-label">Status Mahasiswa</label>
                                                                    <select class="form-control" id="statusInput-{{ $mahasiswa->id_mahasiswa }}" name="status_aktif">
                                                                        <option value="" disabled>Pilih Status</option>
                                                                        <option value="Aktif" {{ $mahasiswa->status_aktif == 'Aktif' ? 'selected' : '' }}>Aktif</option>
                                                                        <option value="Lulus" {{ $mahasiswa->status_aktif == 'Lulus' ? 'selected' : '' }}>Lulus</option>
                                                                        <option value="DO" {{ $mahasiswa->status_aktif == 'DO' ? 'selected' : '' }}>DO</option>
                                                                        <option value="Undur Diri" {{ $mahasiswa->status_aktif == 'Undur Diri' ? 'selected' : '' }}>Undur Diri</option>
                                                                    </select>
                                                                </div>
                                                            </div>
                                                            <div class="modal-footer">
                                            ...
                                            <div class="modal fade" id="confirmDeleteModal" tabindex="-1" aria-labelledby="confirmDeleteLabel" aria-hidden="true">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-body">
                                                            <p>Apakah Anda yakin ingin menghapus mahasiswa <b id="namaMahasiswa"></b>?</p>
                                                        </div>
                                                        <div class="modal-footer">
